...
export default function Home() {
  ...
  const discardSelected = () => { if (!isMyTurn || !hasDrawn || selectedRackIndex===null || !rack[selectedRackIndex]) return reject(); const tile=rack[selectedRackIndex]!; setRack(old=>old.map((value,index)=>index===selectedRackIndex?null:value)); setMyDiscards(old=>[...old,tile]); setPlayerDiscards(old=>({...old,[profileName]:tile})); setSelectedRackIndex(null); setHasDrawn(false); setGame(g=>({...g,siradakiOyuncu:1,tur:g.tur+1})); setNotice("Taş atıldı — robotlar oynuyor"); runRobotTurns(); };
  ...
  return <main className="game-shell">
    ...
    <section className="rack-area"><div className="rack-board"><div className="rack-discard rack-discard-left"><small>Son taş</small>{discard.at(-1)&&<TileView tile={discard.at(-1)!} compact draggable onDragStart={e=>beginDrag(e,"discard",discard.length-1)} onDoubleClick={()=>drawTile("discard")}/>}</div><div className="rack-actions"><button onClick={()=>sortRack("pairs")}>Çift diz</button><button onClick={()=>sortRack("series")}>Seri diz</button></div><div className={`rack-grid ${isDraggingRack?"dragging":""}`} onClick={e=>moveRackByClick(rackIndexFromPointer(e))} onDragOver={e=>e.preventDefault()} onDragEnd={()=>setIsDraggingRack(false)} onDrop={e=>{dropRack(e,0);setIsDraggingRack(false)}}>{rack.slice(0,32).map((tile,i)=><div className={`rack-cell ${selectedRackIndex===i?"selected":""}`} key={i}>{tile&&<TileView tile={tile} draggable={isMyTurn} onDragStart={e=>{e.dataTransfer.effectAllowed="move";beginDrag(e,"rack",i);setIsDraggingRack(true)}}/>}</div>)}</div><div className="rack-discard rack-discard-right" onClick={discardSelected} onDragOver={e=>e.preventDefault()} onDrop={discardTile}><small>Taş at</small><div className="my-discard-stack">{myDiscards.slice(-3).map((tile,i)=><TileView key={tile.id} tile={tile} compact/>)}</div>{myDiscards.length===0&&<span>+</span>}</div></div><p className="rack-status"><span className={isMyTurn?"pulse":""}/>{notice}</p></section>
  </main>;
}
