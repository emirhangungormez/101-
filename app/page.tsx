...
export default function Home() {
  ...
  const [joinedSeats, setJoinedSeats] = useState<number[]>([]);
  ...
  const addComputer = (seat = 0) => setBotSeats(old => old.includes(seat) || old.length >= roomSize ? old : [...old, seat]);
  const removeComputer = (seat = 0) => setBotSeats(old => old.filter(s => s !== seat));
  const joinSeat = (seat = 0) => setJoinedSeats(old => old.includes(seat) ? old : [...old, seat]);
  ...
  if (screen === "room") { const room = rooms.find(r => r.id === selectedRoom) ?? rooms[0]; return <RoomView room={room} botSeats={botSeats} joinedSeats={joinedSeats} onAddComputer={addComputer} onRemoveComputer={removeComputer} onJoinSeat={joinSeat} onStart={() => setScreen("game")} onBack={() => setScreen("lobby")} />; }
...
function RoomView({room,botSeats,joinedSeats,onAddComputer,onRemoveComputer,onJoinSeat,onStart,onBack}:{room:{name:string;owner:string;players:number;max:number;status:string};botSeats:number[];joinedSeats:number[];onAddComputer:(seat:number)=>void;onRemoveComputer:(seat:number)=>void;onJoinSeat:(seat:number)=>void;onStart:()=>void;onBack:()=>void}) { const total = botSeats.length + joinedSeats.length; return <main className="room-screen"><section className="room-content"><div className={`room-profile-grid count-${room.max}`}>{Array.from({length:room.max},(_,i)=>{const bot=botSeats.includes(i), joined=joinedSeats.includes(i); return <div className="room-profile" key={i}><div className={`profile-box ${bot?"bot":""} ${!bot&&!joined?"empty-profile":""}`}>{bot?<><span className="avatar avatar-bot" aria-hidden="true">♟</span><b>Robot {i+1}</b></>:joined?<><span className="avatar" aria-hidden="true">●</span><b>Siz</b></>:<><span className="avatar avatar-empty" aria-hidden="true">+</span><span>Boş profil</span></>}</div>{bot?<button className="robot-add" onClick={()=>onRemoveComputer(i)}>Robot sil</button>:joined?null:<><button className="robot-add room-join-seat" onClick={()=>onJoinSeat(i)}>Katıl</button><button className="robot-add" onClick={()=>onAddComputer(i)}>Robot ekle</button></>}</div>;})}</div><div className="room-actions"><span>{total}/{room.max}</span><button className="start-game" onClick={onStart} disabled={total < 2}>Başlat</button></div></section><button className="back-link room-back" onClick={onBack} aria-label="Geri"><span className="back-arrow">‹</span> Geri</button></main>; }
