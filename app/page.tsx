...
export default function Home() {
  ...
  const removeComputer = () => setBots(old => Math.max(0, old - 1));
  ...
  if (screen === "room") { const room = rooms.find(r => r.id === selectedRoom) ?? rooms[0]; return <RoomView room={room} bots={bots} onAddComputer={addComputer} onRemoveComputer={removeComputer} onJoinSelf={() => setJoinedRoom(true)} onStart={() => setScreen("game")} onBack={() => setScreen("lobby")} />; }
...
function RoomView({room,bots,onAddComputer,onRemoveComputer,onJoinSelf,onStart,onBack}:{room:{name:string;owner:string;players:number;max:number;status:string};bots:number;onAddComputer:()=>void;onRemoveComputer:()=>void;onJoinSelf:()=>void;onStart:()=>void;onBack:()=>void}) { const total = Math.min(room.max, bots); return <main className="room-screen"><section className="room-content"><div className={`room-profile-grid count-${room.max}`}>{Array.from({length:room.max},(_,i)=><div className="room-profile" key={i}><div className={`profile-box ${i<bots?"bot":""}`}>{i<bots?<b>Robot {i+1}</b>:<span>Boş profil</span>}</div>{i<bots?<button className="robot-add" onClick={onRemoveComputer}>Robot sil</button>:<><button className="robot-add room-join-seat" onClick={onJoinSelf}>Katıl</button><button className="robot-add" onClick={onAddComputer}>Robot ekle</button></>}</div>)}</div><div className="room-actions"><span>{total}/{room.max}</span><button className="start-game" onClick={onStart} disabled={total < 2}>Başlat</button></div></section><button className="back-link room-back" onClick={onBack} aria-label="Geri"><span className="back-arrow">‹</span> Geri</button></main>; }
