...
export default function Home() {
  ...
  const [roomSize, setRoomSize] = useState<1 | 2 | 3 | 4>(4);
...
function Lobby({rooms,roomName,setRoomName,roomSize,setRoomSize,onCreate,onJoin,onBack}:{rooms:{id:number;name:string;owner:string;players:number;max:number;status:string}[];roomName:string;setRoomName:(v:string)=>void;roomSize:1|2|3|4;setRoomSize:(v:1|2|3|4)=>void;onCreate:()=>void;onJoin:(id:number)=>void;onBack:()=>void}) { return <main className="lobby-screen"><button className="back-link lobby-back" onClick={onBack} aria-label="Geri"><span className="back-arrow">‹</span> Geri</button><section className="lobby-content"><div className="create-panel"><h1>Oyun oluştur</h1><input aria-label="Oda adı" value={roomName} onChange={e=>setRoomName(e.target.value)} /><div className="tile-picker" aria-label="Oyuncu sayısı">{[1,2,3,4].map(value=><button key={value} className={`player-tile tile-choice-${value} ${roomSize===value?"selected":""}`} onClick={()=>setRoomSize(value as 1|2|3|4)}><span className="tile-pips">{Array.from({length:value},(_,i)=><i key={i}/>)}</span><b>{value}</b></button>)}</div><button className="create-button" onClick={onCreate}>Oluştur</button></div><div className="lobby-divider"/><div className="rooms-panel"><h1>Odalar</h1><div className="room-list">{rooms.length === 0 ? <p className="rooms-footnote">Henüz oda yok.</p> : rooms.map(room=><button className="room-row" key={room.id} onClick={()=>onJoin(room.id)}><span className="room-dot"/><span className="room-main"><b>{room.name}</b></span><span className="room-count">{room.players}/{room.max}</span><span className="room-status">→</span></button>)}</div></div></section></main>; }
