...
function StartMenu({onStart,onSettings}:{onStart:()=>void;onSettings:()=>void;notice:string}) { return <main className="start-screen"><nav className="menu-actions" aria-label="Ana menü"><button className="primary-action" onClick={onStart}>Başla</button><button className="ghost-action" onClick={onSettings}>Ayarlar</button></nav></main>; }

function Lobby({rooms,roomName,setRoomName,roomSize,setRoomSize,onCreate,onJoin,onBack}:{rooms:{id:number;name:string;owner:string;players:number;max:number;status:string}[];roomName:string;setRoomName:(v:string)=>void;roomSize:2|4;setRoomSize:(v:2|4)=>void;onCreate:()=>void;onJoin:(id:number)=>void;onBack:()=>void}) { return <main className="lobby-screen"><section className="lobby-content"><div className="create-panel"><h1>Oyun oluştur</h1><input aria-label="Oda adı" value={roomName} onChange={e=>setRoomName(e.target.value)} /><div className="size-picker"><button className={roomSize===2?"selected":""} onClick={()=>setRoomSize(2)}>2</button><button className={roomSize===4?"selected":""} onClick={()=>setRoomSize(4)}>4</button></div><button className="create-button" onClick={onCreate}>Oluştur</button><button className="back-link" onClick={onBack}>← Geri</button></div><div className="lobby-divider"/><div className="rooms-panel"><h1>Odalar</h1><div className="room-list">{rooms.length === 0 ? <p className="rooms-footnote">Henüz oda yok.</p> : rooms.map(room=><button className="room-row" key={room.id} onClick={()=>onJoin(room.id)}><span className="room-dot"/><span className="room-main"><b>{room.name}</b></span><span className="room-count">{room.players}/{room.max}</span><span className="room-status">→</span></button>)}</div></div></section></main>; }
