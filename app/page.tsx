...
export default function Home() {
  ...
  return <main className="game-shell">
    ...
    <section className="rack-area"><div className="rack-toolbar"><button onClick={()=>sortRack("pairs")}>Çift diz</button><button onClick={()=>sortRack("series")}>Seri diz</button></div><div className="rack-board"><div className="rack-discard rack-discard-left"><small>Son taş</small>{discard.at(-1)&&<TileView tile={discard.at(-1)!} compact draggable onDragStart={e=>beginDrag(e,"discard",discard.length-1)} onDoubleClick={()=>drawTile("discard")}/>}</div><div className="rack-grid">{rack.slice(0,32).map((tile,i)=><div className="rack-cell" key={i} onDragOver={e=>e.preventDefault()} onDrop={e=>dropRack(e,i)}>{tile&&<TileView tile={tile} draggable={isMyTurn} onDragStart={e=>beginDrag(e,"rack",i)}/>}</div>)}</div><div className="rack-discard rack-discard-right" onDragOver={e=>e.preventDefault()} onDrop={discardTile}><small>Taş at</small><span>+</span></div></div><p className="rack-status"><span className={isMyTurn?"pulse":""}/>{notice}</p></section>
  </main>;
}
