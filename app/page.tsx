...
export default function Home() {
  ...
  const discardRackIndex = (rackIndex:number) => { if (!isMyTurn || !hasDrawn || !rack[rackIndex]) return reject(); const tile=rack[rackIndex]!; setRack(old=>old.map((value,index)=>index===rackIndex?null:value)); setMyDiscards(old=>[...old,tile]); setPlayerDiscards(old=>({...old,[profileName]:tile})); setSelectedRackIndex(null); setHasDrawn(false); setGame(g=>({...g,siradakiOyuncu:1,tur:g.tur+1})); setNotice("Taş atıldı — robotlar oynuyor"); runRobotTurns(); };
