...
function RoomView({room,currentSocketId,onAddComputer,onRemoveComputer,onJoinSeat,onLeaveSeat,onStart,onBack}:{room:any;currentSocketId:string;onAddComputer:(seat:number)=>void;onRemoveComputer:(seat:number)=>void;onJoinSeat:(seat:number)=>void;onLeaveSeat:()=>void;onStart:()=>void;onBack:()=>void}) { const max=room?.maksimum ?? room?.max ?? 4, players=room?.oyuncular ?? [], total=players.length, mine=players.find((p:any)=>p.socketId===currentSocketId); return <main className="room-screen"><section className="room-content"><div className={`room-profile-grid count-${max}`}>{Array.from({length:max},(_,i)=>{const player=players.find((p:any)=>p.koltukNo===i); return <div className="room-profile" key={i}><div className={`profile-box ${player?.bot?"bot":""} ${player?.socketId===currentSocketId?"player":""} ${!player?"empty-profile":""}`}>{player?.bot?<><span className="avatar avatar-bot" aria-hidden="true">🤖</span><b>Robot</b></>:player?<><span className="avatar avatar-player" aria-hidden="true">{player.avatar||"🙂"}</span><b>{player.isim}</b></>:<><span className="avatar avatar-empty" aria-hidden="true">+</span><span>Boş profil</span></>}</div>{player?.bot?<button className="robot-add" onClick={()=>onRemoveComputer(i)}>Robot sil</button>:player?.socketId===currentSocketId?<button className="robot-add room-leave-seat" onClick={onLeaveSeat}>Ayrıl</button>:!player&&<><button className="robot-add room-join-seat" onClick={()=>onJoinSeat(i)} disabled={Boolean(mine)}>Katıl</button><button className="robot-add" onClick={()=>onAddComputer(i)}>Robot ekle</button></>}</div>;})}</div><div className="room-actions"><span>{total}/{max}</span><button className="start-game" onClick={onStart} disabled={total!==max}>Başlat</button></div></section><button className="back-link room-back" onClick={onBack} aria-label="Geri"><span className="back-arrow">‹</span> Geri</button></main>; }
