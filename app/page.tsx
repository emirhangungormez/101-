...
export default function Home() {
  ...
  const createRoom = () => { setJoinedRoom(false); if (socket?.connected) { socket.emit("oda-olustur", roomName || "Yeni Masa"); return; } const id = Date.now(); setRooms(old => [...old, { id, name: roomName || "Yeni Masa", owner: "Siz", players: 0, max: roomSize, status: "" }]); setSelectedRoom(id); setBots(0); setScreen("room"); };
  const deleteRoom = (id: number) => setRooms(old => old.filter(room => !(room.id === id && room.owner === "Siz")));
  const joinRoom = (id: number) => { setJoinedRoom(true); if (socket?.connected) socket.emit("oda-katil", id); setSelectedRoom(id); setScreen("room"); };
  const addComputer = (seat = 0) => setBotSeats(old => { if (old.includes(seat) || old.length >= roomSize) return old; setRooms(rooms => rooms.map(room => room.id === selectedRoom ? { ...room, players: room.players + 1 } : room)); return [...old, seat]; });
  const removeComputer = (seat = 0) => setBotSeats(old => { if (!old.includes(seat)) return old; setRooms(rooms => rooms.map(room => room.id === selectedRoom ? { ...room, players: Math.max(0, room.players - 1) } : room)); return old.filter(s => s !== seat); });
  ...
  if (screen === "room") { const room = rooms.find(r => r.id === selectedRoom) ?? rooms[0]; return <RoomView room={room} playerName={profileName} playerEmoji={profileEmoji} botSeats={botSeats} joinedSeat={joinedSeat} onAddComputer={addComputer} onRemoveComputer={removeComputer} onJoinSeat={joinSeat} onLeaveSeat={leaveSeat} onStart={() => setScreen("game")} onBack={() => setScreen("lobby")} />; }
...
function RoomView({room,playerName,playerEmoji,botSeats,joinedSeat,onAddComputer,onRemoveComputer,onJoinSeat,onLeaveSeat,onStart,onBack}:{room:{name:string;owner:string;players:number;max:number;status:string};playerName:string;playerEmoji:string;botSeats:number[];joinedSeat:number|null;onAddComputer:(seat:number)=>void;onRemoveComputer:(seat:number)=>void;onJoinSeat:(seat:number)=>void;onLeaveSeat:()=>void;onStart:()=>void;onBack:()=>void}) { const total = botSeats.length + (joinedSeat === null ? 0 : 1); return <main className="room-screen"><section className="room-content"><div className={`room-profile-grid count-${room.max}`}>{Array.from({length:room.max},(_,i)=>{const bot=botSeats.includes(i), joined=joinedSeat===i, seatTaken=joinedSeat!==null; return <div className="room-profile" key={i}><div className={`profile-box ${bot?"bot":""} ${joined?"player":""} ${!bot&&!joined?"empty-profile":""}`}>{bot?<><span className="avatar avatar-bot" aria-hidden="true">🤖</span><b>Robot</b></>:joined?<><span className="avatar avatar-player" aria-hidden="true">{playerEmoji}</span><b>{playerName}</b></>:<><span className="avatar avatar-empty" aria-hidden="true">+</span><span>Boş profil</span></>}</div>{bot?<button className="robot-add" onClick={()=>onRemoveComputer(i)}>Robot sil</button>:joined?<button className="robot-add room-leave-seat" onClick={onLeaveSeat}>Ayrıl</button>:<><button className="robot-add room-join-seat" onClick={()=>onJoinSeat(i)} disabled={seatTaken}>Katıl</button><button className="robot-add" onClick={()=>onAddComputer(i)}>Robot ekle</button></>}</div>;})}</div><div className="room-actions"><span>{total}/{room.max}</span><button className="start-game" onClick={onStart} disabled={total !== room.max}>Başlat</button></div></section><button className="back-link room-back" onClick={onBack} aria-label="Geri"><span className="back-arrow">‹</span> Geri</button></main>; }
