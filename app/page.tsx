...
export default function Home() {
  ...
  const [incomingDiscards, setIncomingDiscards] = useState<Tile[]>([{ id: 109, value: 8, color: "blue" }, { id: 110, value: 4, color: "yellow" }, { id: 111, value: 12, color: "red" }]);
  const [selectedRackIndex, setSelectedRackIndex] = useState<number | null>(null);
  const [rackPointerDrag, setRackPointerDrag] = useState<{source:"rack"|"discard";index:number;tile:Tile;x:number;y:number;centerOffsetX:number;centerOffsetY:number}|null>(null);
  ...
  const discardRackIndex = (rackIndex:number) => { if (!isMyTurn || !rack[rackIndex]) return reject(); if (!hasDrawn) return setNotice("Önce taş çekmelisin"); const tile=rack[rackIndex]!; setRack(old=>old.map((value,index)=>index===rackIndex?null:value)); setMyDiscards(old=>[...old,tile]); setPlayerDiscards(old=>({...old,[profileName]:tile})); setSelectedRackIndex(null); setHasDrawn(false); setTurnPhase("draw"); setTurnSeconds(60); setGame(g=>({...g,tur:g.tur+1})); setNotice("Taş atıldı, taş çekmelisin"); };
  const discardSelected = () => { if (selectedRackIndex===null) return reject(); discardRackIndex(selectedRackIndex); };
  const startRackPointerDrag = (e:React.PointerEvent<HTMLDivElement>,source:"rack"|"discard",index:number,tile:Tile) => { if (e.button!==0) return; const rect=e.currentTarget.getBoundingClientRect(); e.currentTarget.setPointerCapture(e.pointerId); e.preventDefault(); setRackPointerDrag({source,index,tile,x:e.clientX,y:e.clientY,centerOffsetX:rect.left+rect.width/2-e.clientX,centerOffsetY:rect.top+rect.height/2-e.clientY}); };
  ...
  useEffect(() => { if (screen!=="game") return; if (turnSeconds>0) { const timer=window.setTimeout(()=>setTurnSeconds(value=>value-1),1000); return ()=>window.clearTimeout(timer); } if (turnPhase==="draw") { drawTile("deck"); setNotice("Süre doldu, taş otomatik çekildi"); } else { const index=rack.findIndex(Boolean); if (index>=0) { discardRackIndex(index); setNotice("Süre doldu, taş otomatik atıldı"); } } }, [screen,turnSeconds,turnPhase]);
