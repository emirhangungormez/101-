...
export default function Home() {
  ...
  const [rackPointerDrag, setRackPointerDrag] = useState<{index:number;tile:Tile;x:number;y:number;centerOffsetX:number;centerOffsetY:number}|null>(null);
  ...
  const startRackPointerDrag = (e:React.PointerEvent<HTMLDivElement>,index:number,tile:Tile) => { if (e.button!==0) return; const rect=e.currentTarget.getBoundingClientRect(); e.currentTarget.setPointerCapture(e.pointerId); e.preventDefault(); setRackPointerDrag({index,tile,x:e.clientX,y:e.clientY,centerOffsetX:rect.left+rect.width/2-e.clientX,centerOffsetY:rect.top+rect.height/2-e.clientY}); };
