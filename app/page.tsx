...
export default function Home() {
  ...
  const discardRackIndex = (rackIndex:number) => { if (!isMyTurn || !rack[rackIndex]) return reject(); const tile=rack[rackIndex]!; setRack(old=>old.map((value,index)=>index===rackIndex?null:value)); setMyDiscards(old=>[...old,tile]); setPlayerDiscards(old=>({...old,[profileName]:tile})); setSelectedRackIndex(null); setHasDrawn(false); setGame(g=>({...g,tur:g.tur+1})); setNotice("Taş atıldı"); };
