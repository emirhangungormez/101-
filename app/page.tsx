...
function RoomView({room,botSeats,joinedSeat,onAddComputer,onRemoveComputer,onJoinSeat,onLeaveSeat,onStart,onBack}:{room:{name:string;owner:string;players:number;max:number;status:string};botSeats:number[];joinedSeat:number|null;onAddComputer:(seat:number)=>void;onRemoveComputer:(seat:number)=>void;onJoinSeat:(seat:number)=>void;onLeaveSeat:()=>void;onStart:()=>void;onBack:()=>void}) { const total = botSeats.length + (joinedSeat === null ? 0 : 1); return <main className="room-screen"><section className="room-content"><div className={`room-profile-grid count-${room.max}`}>{Array.from({length:room.max},(_,i)=>{const bot=botSeats.includes(i), joined=joinedSeat===i, seatTaken=joinedSeat!==null; return <div className="room-profile" key={i}><div className={`profile-box ${bot?"bot":""} ${joined?"player":""} ${!bot&&!joined?"empty-profile":""}`}>{bot?<><span className="avatar avatar-bot" aria-hidden="true">🤖</span><b>Robot</b></>:joined?<><span className="avatar avatar-player" aria-hidden="true">●</span><b>Siz</b></>:<><span className="avatar avatar-empty" aria-hidden="true">+</span><span>Boş profil</span></>}</div>{bot?<button className="robot-add" onClick={()=>onRemoveComputer(i)}>Robot sil</button>:joined?<button className="robot-add room-leave-seat" onClick={onLeaveSeat}>Ayrıl</button>:<><button className="robot-add room-join-seat" onClick={()=>onJoinSeat(i)} disabled={seatTaken}>Katıl</button><button className="robot-add" onClick={()=>onAddComputer(i)}>Robot ekle</button></>}</div>;})}</div><div className="room-actions"><span>{total}/{room.max}</span><button className="start-game" onClick={onStart} disabled={total < 2}>Başlat</button></div></section><button className="back-link room-back" onClick={onBack} aria-label="Geri"><span className="back-arrow">‹</span> Geri</button></main>; }
