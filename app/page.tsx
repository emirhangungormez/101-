...
function Lobby({rooms,roomName,setRoomName,roomSize,setRoomSize,onCreate,onJoin,onBack}:{rooms:{id:number;name:string;owner:string;players:number;max:number;status:string}[];roomName:string;setRoomName:(v:string)=>void;roomSize:2|3|4;setRoomSize:(v:2|3|4)=>void;onCreate:()=>void;onJoin:(id:number)=>void;onBack:()=>void}) { return <main className="lobby-screen"><button className="back-link lobby-back" onClick={onBack} aria-label="Geri"><span className="back-arrow">‹</span> Geri</button><section className="lobby-content"><div className="create-panel"><h1>Oyun oluştur</h1><input aria-label="Oda adı" value={roomName} onChange={e=>setRoomName(e.target.value)} /><div className="size-picker"><button className={roomSize===2?"selected":""} onClick={()=>setRoomSize(2)}>2</button><button className={roomSize===3?"selected":""} onClick={()=>setRoomSize(3)}>3</button><button className={roomSize===4?"selected":""} onClick={()=>setRoomSize(4)}>4</button></div><div className="board-preview" aria-label={`${roomSize} kişilik Okey masası önizlemesi`}><div className="board-surface"><div className="board-tiles">{Array.from({length:10},(_,i)=><i key={i}/>)}</div></div><div className="board-seats">{Array.from({length:roomSize},(_,i)=><span key={i} className={`seat-${i} ${i===0?"active":""}`}>{i+1}</span>)}</div></div><button className="create-button" onClick={onCreate}>Oluştur</button></div><div className="lobby-divider"/><div className="rooms-panel"><h1>Odalar</h1><div className="room-list">{rooms.length === 0 ? <p className="rooms-footnote">Henüz oda yok.</p> : rooms.map(room=><button className="room-row" key={room.id} onClick={()=>onJoin(room.id)}><span className="room-dot"/><span className="room-main"><b>{room.name}</b></span><span className="room-count">{room.players}/{room.max}</span><span className="room-status">→</span></button>)}</div></div></section></main>; }

function RoomView({room,bots,onAddComputer,onStart,onBack}:{room:{name:string;owner:string;players:number;max:number;status:string};bots:number;onAddComputer:()=>void;onStart:()=>void;onBack:()=>void}) { const total = Math.min(room.max, 1 + bots); return <main className="room-screen"><header className="lobby-top"><button className="back-link" onClick={onBack}>←</button><b>{room.name}</b><span>{total}/{room.max}</span></header><section className="room-content"><div className="seat-grid">{Array.from({length:room.max},(_,i)=>i===0?<div className="seat occupied" key={i}><b>Siz</b></div>:i<=bots?<div className="seat occupied" key={i}><b>Bot {i}</b></div>:<button className="seat empty" key={i} onClick={onAddComputer}><strong>+</strong><b>Bot</b></button>)}</div><div className="room-actions"><span>{total}/{room.max}</span><button className="start-game" onClick={onStart} disabled={total < 2}>Başlat</button></div></section></main>; }
