...
function Lobby({rooms,roomName,setRoomName,roomSize,setRoomSize,onCreate,onJoin,onBack}:{rooms:{id:number;name:string;owner:string;players:number;max:number;status:string}[];roomName:string;setRoomName:(v:string)=>void;roomSize:2|4;setRoomSize:(v:2|4)=>void;onCreate:()=>void;onJoin:(id:number)=>void;onBack:()=>void}) { return <main className="lobby-screen"><header className="lobby-top"><button className="back-link" onClick={onBack}>←</button><b>Odalar</b><span/></header><section className="lobby-content"><div className="create-panel"><input aria-label="Oda adı" value={roomName} onChange={e=>setRoomName(e.target.value)} /><div className="size-picker"><button className={roomSize===2?"selected":""} onClick={()=>setRoomSize(2)}>2</button><button className={roomSize===4?"selected":""} onClick={()=>setRoomSize(4)}>4</button></div><button className="create-button" onClick={onCreate}>Oluştur</button></div><div className="lobby-divider"/><div className="rooms-panel"><div className="room-list">{rooms.map(room=><button className="room-row" key={room.id} onClick={()=>onJoin(room.id)}><span className="room-dot"/><span className="room-main"><b>{room.name}</b></span><span className="room-count">{room.players}/{room.max}</span><span className="room-status">→</span></button>)}</div></div></section></main>; }
