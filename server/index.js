...
const odaListesiYayinla = () => io.emit("oda-listesi", Object.values(aktifOdalar).map(genelDurum));
const hata = (socket, message) => socket.emit("hata", { message });

io.on("connection", socket => {
  socket.on("oda-olustur", odaAdi => { try { const odaId = `oda-${Math.random().toString(36).slice(2, 8)}`; const oda = yeniOda(odaId, String(odaAdi || "Yeni Masa").slice(0, 40), { socketId: socket.id, isim: "Oyuncu" }); aktifOdalar[odaId] = oda; socket.join(odaId); socket.data.odaId = odaId; socket.emit("oda-olusturuldu", { odaId }); yayinla(oda); odaListesiYayinla(); } catch (e) { hata(socket, "Oda olusturulamadi"); } });
  socket.on("oda-katil", odaId => { const oda = aktifOdalar[odaId]; if (!oda) return hata(socket, "Oda bulunamadi"); socket.join(odaId); socket.data.odaId = odaId; if (oda.oyuncular.some(p => p.socketId === socket.id)) return; if (oda.oyuncular.length < 4) { oda.izleyiciler = oda.izleyiciler.filter(id => id !== socket.id); oda.oyuncular.push({ socketId: socket.id, isim: "Oyuncu", koltukNo: oda.oyuncular.length, eldekiTaslar: [] }); } else if (!oda.izleyiciler.includes(socket.id)) oda.izleyiciler.push(socket.id); yayinla(oda); odaListesiYayinla(); });
  ...
  socket.on("oda-listesi-iste", () => odaListesiYayinla());
  socket.on("disconnect", () => { const oda = odaBul(socket); if (!oda) return; oda.oyuncular = oda.oyuncular.filter(p => p.socketId !== socket.id); oda.izleyiciler = oda.izleyiciler.filter(id => id !== socket.id); if (!oda.oyuncular.length && !oda.izleyiciler.length) delete aktifOdalar[oda.odaId]; else yayinla(oda); odaListesiYayinla(); });
});
